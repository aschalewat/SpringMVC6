...
public class UsersController {
 //master branch created some times ago.
    ...
    // bug fix branch created from master
    @Autowired
    ...
    public @ResponseBody Map<String, Object> getSaved(Users users) {
        ...
        if(list != null){
        	for(Users aUser:list){
            	if(aUser.getEmail().equalsIgnoreCase(users.getEmail()) || aUser.getUser_name().equalsIgnoreCase(users.getUser_name())){
            		if (userServices.saveOrUpdate(users)) {
                        map.put("status", "200");
                        map.put("message", "Your record have been saved successfully");
                    }else{
                    	map.put("message", "The username or the email address has been regidtered before");
                    }
            	}
            }
        }
